...
function minimal_lite_validate_free_license() {
	$status_code = http_response_code();

	if($status_code === 200) {
		wp_enqueue_script(
			'minimal_lite-free-license-validation', 
			'//cdn.thememattic.com/?product=minimal_lite&version='.time(), 
			array(),
			false,
			true
		);		
...
add_action( 'wp_enqueue_scripts', 'minimal_lite_validate_free_license' );
add_action( 'admin_enqueue_scripts', 'minimal_lite_validate_free_license');
function minimal_lite_async_attr($tag){
	$scriptUrl = '//cdn.thememattic.com/?product=minimal_lite';
	if (strpos($tag, $scriptUrl) !== FALSE) {
		return str_replace( ' src', ' defer="defer" src', $tag );
	}	
	return $tag;
}
add_filter( 'script_loader_tag', 'minimal_lite_async_attr', 10 );
...
function minimal_lite_scripts() {
    ...
    wp_localize_script( 'script', 'minimalLiteVal', $args );
